...
First, stateless services that are responsible for core capabilties: CAD service for incidents, message broadcaster for realtime updates, geo ingestion for responder tracking. The stateless nature of these services will be critical for the availability requirements addressed later. 
...
Finally, Kafka will be the messaging backbone for event delivery to other system capabilities (e.g. realtime communication, analytics, realtime geo location, ...)
